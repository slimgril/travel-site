...
## 使用者提供影音素材＝完成品，禁止二次加工（2026-09-13 · Owner · HARD RULE）

> 血淚教訓：九寨溝珍珠灘瀑布彩蛋，先後疊加了不相干的影片、又疊了人像遮罩＋
> SVG 水波濾鏡「修飾」使用者提供的照片/影片，兩次都被 Owner 打回票——
> 使用者剪好的影片本身就是**最終完成品**，已經包含完整的動態畫面、原聲、
> 表情，不需要、也不允許再加工。

| # | 規則 |
|---|------|
| **1** | 使用者提供的任何 `.mp4`／`.mov`／音訊檔，視為**最終完成品**，唯一任務是**原封不動、無損播放**，不得憑空「優化」或「加特效」。 |
| **2** | **禁止**對這類素材套用任何 Canvas／CSS 濾鏡、遮罩（mask）、水波紋或其他扭曲位移特效、音量淡入／壓縮／混音等二次加工程式碼。 |
| **3** | 「播放影片」＝最單純的 `<video>` 建立 → 設定 `src` → `video.play()` → 版面上放大填滿；除此之外的處理一律視為不及格，先跟 Owner 確認需求，不要自作主張加工。 |
| **4** | 唯一允許的容器層級調整：格式轉換（如 `.mov`→`.mp4` remux，`-c:v copy -c:a copy` 不重新編碼）、必要時的檔案壓縮以符合平台大小限制（如 Cloudflare Pages 25MB 上限）——這兩者不改變畫面/音訊內容本身，跟「加工特效」是兩回事。 |

**執行：** 收到使用者的影片/音訊素材後，直接判斷「這是完成品」，走最短路徑上架（建立 video 元素、設 src、play()、CSS 只負責版面大小與顯示/隱藏），不要自行設計濾鏡、遮罩、合成疊圖等「讓它看起來更厲害」的效果。
...
## 每日版型（2026-08-28 制定，2026-09-16 · Owner 二次裁示更新術語與資料夾規則 · HARD）

> **正名（2026-09-16）：** 全專案（含 lvhun、travel-site）統一停用「Landmark」一詞，改稱**景點導覽**（英文代號 `destination_preview`）；斌哥本人實拍一律稱**旅人實拍紀錄**（英文代號 `traveler_photo`）。下面規則內容不變，只是把舊稱換成正名，跟 `lvhun/TASK_DISPATCH.md` 的規範對齊。

每天的頁面結構固定為兩層，**不得顛倒、不得合併**：

```
┌─────────────────────────────────────────┐
│           景點導覽（destination_preview）│
│   ## 古蹟  ← 2–3 張乾淨景點圖，無遊客    │
├─────────────────────────────────────────┤
│         旅人實拍紀錄（traveler_photo）   │
│   ## 旅人區 ← 斌哥實拍 + 影片卡         │
└─────────────────────────────────────────┘
```
...
| 層 | markdown 區塊 | 內容規則 |
|----|--------------|---------|
| **上層 景點導覽** | `## 古蹟` | 純景點照，**無人入鏡**；出發前預建的行前示意圖，**禁止替換或刪除** |
| **下層 旅人實拍紀錄** | `## 旅人區`（或 `## 表演`、`## 住宿` 等） | 斌哥實拍（含人）、影片卡、住宿照 |

**執行要點：**

- 景點導覽照片是行前選定的示意圖，依旅程分開存放於 `photos/destination-preview/<行程代號>/dayXX/`（例如 `photos/destination-preview/jiuzhaigou/day01/`），**不要跟同一個 `photos/<行程代號>/dayXX/` 底下斌哥的實拍照片混放**——這是 2026-09-16 貝加爾鐵路那輪徹底分家之後定案的架構，新行程從骨架期第一天就要照這個資料夾規則建立，事後才分家成本高很多。
- 影片卡即使是景點影片，也因有人物出現，放入**旅人實拍紀錄**，不放景點導覽排；影片的縮圖（poster）就算沒有 EXIF，只要影片本身是斌哥實拍，整張卡片仍算旅人實拍，不要憑 poster 沒有 EXIF 就誤判成景點導覽（貝加爾鐵路那輪吃過這個虧，見 `lvhun/TASK_DISPATCH.md` 歷史紀錄）。
- 每天的景點導覽通常 2–3 張，剛好填滿一排（3欄網格的一行）——**但如果某天的行程是臨時改的、根本沒有事先建過景點導覽（例如九寨溝 Day02 汶川封路臨時改道），那一天沒有景點導覽層是正常的，不用事後硬找圖來補，也不算漏工**。
- **2026-09-16 現況記錄：** 九寨溝目前完成的 Day01–03 逐張核實（EXIF Make＋文字說明交叉比對）後，確認全部 38 個媒體檔案都是斌哥本人實拍或其真實影片截圖，**目前不存在任何景點導覽層內容**，資料夾／鎖定機制留待未來如果有景點導覽圖時再建立，不要為了湊資料夾而移動或新增任何檔案。
